...
class Artifact:
    """Un dépôt HF à matérialiser sous ``modelstore/<local_dir>``."""
...
# Manifeste des poids requis. Patterns larges : poids + config + scalers/climato.
MANIFEST: tuple[Artifact, ...] = (
    Artifact(
        key="backbone",
        repo_id="Prithvi-WxC/prithvi.wxc.2300m.v1",
        local_dir="prithvi-wxc-2300m",
        patterns=["*.pt", "*.yaml", "*.json", "climatology/*", "*.nc"],
        note="Backbone foundation (2,3 B) + config + scalers de normalisation",
    ),
    Artifact(
        key="granite",
        ...
        note="Adapter downscaling IBM Granite (×12) — démarrage sans fine-tuning",
    ),
)
